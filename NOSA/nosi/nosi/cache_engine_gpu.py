...
class CacheEngine:
    def __init__(self,
        ...
        # H2D stats (gated by profiler; mirrors cache_engine.py)
    ...
    def reset_h2d_stats(self):
        """Reset H2D stats accumulators (no-op for GPU-only path, no real H2D)."""
    ...
    def get_h2d_stats(self, _sync=True):
        """Return H2D stats dict (GPU-only path: stats are always zero)."""
        if _sync:
        ...
        return {
            ...
            "wasted_reload": max(0, miss - true_miss) if true_total > 0 else 0,
    ...
    def prepare_prefill(self, total_bsz, seq_len):
        ...
        H = self.head_num
        D = self.head_dim
        kv_shape = (total_bsz, seq_len + self.max_gen_len, H, 2, D)
        ...
        if (
            self._kv_cpu is None
            or self._kv_cpu.shape != kv_shape
            or self._kv_cpu.dtype != self.dtype
            or self._kv_cpu.device != self.device
        ):
            self._kv_cpu = torch.empty(kv_shape, dtype=self.dtype, device=self.device)
        self._k_cpu = self._kv_cpu[..., 0, :]
        self._v_cpu = self._kv_cpu[..., 1, :]
        self._kv_cpu_flat_packed = self._kv_cpu.view(-1, 2 * D)
    ...
    def decode_update_has_kv_bias(self, key_states, value_states, kv_bias, topk_idx):
        ...
        # print(self._load_mask.min(), self._load_mask.max(), self._k_gpu.isnan().any(), self._v_gpu.isnan().any())
        # print("load block num:", (self._load_mask > 0).sum())
    ...
    def dense_decode_update(self, key_states, value_states):
        ...
        return (
            self._k_cpu[:, :self.seq_length, :, :].contiguous(),
            self._v_cpu[:, :self.seq_length, :, :].contiguous(),
            cache_lens,
        )
...
class InfLLMv2Cache(DynamicCache):
    def __init__(self,
                 config, num_hidden_layers: Optional[int] = None,
                 has_kv_bias: bool = False, max_gen_len: int = 8192) -> None:
        super().__init__(config=config)
        self.layers = [
            InfLLMv2CacheLayer(config, has_kv_bias, max_gen_len=max_gen_len)
    ...
    def decode_update_kv(self, key_states, value_states, kv_bias, layer_idx, topk_idx, skip_offload=False):
        if layer_idx == 0:
            self._seen_tokens += key_states.shape[-2]
        return self.layers[layer_idx].decode_update_kv(
            key_states, value_states, kv_bias, topk_idx, skip_offload=skip_offload
        )
    ...
    # ── H2D stats (mirrors cache_engine.py DynamicCache API) ──────────
    ...
    def get_h2d_stats(self):
        ...
        h2d_bytes = sum(s["h2d_bytes"] for s in per_layer)
        hr = 1.0 - miss / total if total > 0 else 0.0
        return {
            "miss_blocks": miss, "total_blocks": total,
            "hit_rate": hr,
            "h2d_bytes": h2d_bytes,
            "true_miss": 0, "wasted_reload": 0,
            "block_size": self.layers[0].cache_engine.block_size if self.layers else 64,
            "head_dim": self.layers[0].cache_engine.head_dim if self.layers else 128,
            "elem_size": 2,
