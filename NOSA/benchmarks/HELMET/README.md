...
For the SparDA release, run HELMET from the root SparDA Docker image. Standalone HELMET dependency installation and API-model extras are not supported by this release. The local `requirements.txt` records the Docker-tested dependency versions for source/SBOM scans.
...
### Container and accelerator environments

This release does not bundle the upstream Gaudi/vLLM container scripts. Run
HELMET from the local environment or from a NOSA container built with the flow in
`NOSA/dependencies/README.md`. If you need a separate Gaudi/vLLM setup, build it
from the official vLLM/Gaudi instructions and point HELMET at that server with
