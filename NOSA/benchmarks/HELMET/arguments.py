...
def parse_arguments():
    ...
    parser.add_argument("--sparda", dest="enable_sparda",
                        action="store_true", help="enable SparDA indexer for NOSA models; use with --indexer_path")
    parser.add_argument("--enable_infinigen", action="store_true", help="enable InfiniGen decode (InfLLMv2 sparse prefill + InfiniGen masked decode)")
    parser.add_argument("--indexer_path", type=str, default=None, help="path to trained indexer checkpoint for SparDA mode (--sparda)")
    parser.add_argument("--long_context", action="store_true", help="enable long-context extension (LongRoPE for InfLLMv2, rope_theta increase for NOSA)")
    parser.add_argument("--yarn", action="store_true", help="use YaRN instead of rope_theta increase for NOSA long-context")

    # single-model mode (set by run.py)
    parser.add_argument("--model_key", type=str, default=None, help="model key for single-model evaluation (e.g. 8b_nosa_sft)")
    ...
    # block hit-rate profiling
    parser.add_argument("--collect_block_hit_rate", action="store_true",
                        help="Collect block hit-rate stats per task during decode "
                             "(NOSA/InfLLMv2 sparse models only)")
