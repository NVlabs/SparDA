...
void set_params_fprop(Flash_fwd_params &params,
    ...
    params.kv_bias = kv_bias.data_ptr();
    ...
    params.kv_bias_row_stride = kv_bias.stride(-2);
    params.kv_bias_head_stride = kv_bias.stride(-1);
    ...
    if (cu_seqlens_q_d == nullptr) {
        ...
        params.kv_bias_batch_stride = kv_bias.stride(0);
...
mha_fwd_kvcache(at::Tensor &q,                 // batch_size x seqlen_q x num_heads x head_size
                ...
                const at::Tensor &bcache,            // batch_size_c x seqlen_k x num_heads_k 
    ...
    TORCH_CHECK(bcache.dtype() == q_dtype, "query and bias must have the same dtype");

    CHECK_DEVICE(q); CHECK_DEVICE(kcache); CHECK_DEVICE(vcache); CHECK_DEVICE(bcache);

    TORCH_CHECK(q.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(kcache.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(vcache.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    TORCH_CHECK(bcache.stride(-1) == 1, "Input tensor must have contiguous last dimension");
    ...
    if (!paged_KV) {
        ...
        CHECK_SHAPE(bcache, batch_size_c, seqlen_k, num_heads_k);
    } else {
        CHECK_SHAPE(kcache, num_blocks, page_block_size, num_heads_k, head_size_og);
        CHECK_SHAPE(vcache, num_blocks, page_block_size, num_heads_k, head_size_og);
        CHECK_SHAPE(bcache, num_blocks, page_block_size, num_heads_k);
    ...
    set_params_fprop(params,
                     ...
                     q_padded, kcache_padded, vcache_padded, bcache, out,
