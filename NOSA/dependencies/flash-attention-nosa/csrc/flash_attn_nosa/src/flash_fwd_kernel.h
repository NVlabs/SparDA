...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        // add kv_bias
        Tensor acc_s_4d = make_tensor(acc_s.data(),flash::convert_layout_acc_rowcol(acc_s.layout()));
        int _lane_id = threadIdx.x % 32;
        int _col_idx_offset = n_block * kBlockN + (_lane_id % 4) * 2;
        int _row_idx_offset = m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4;
        int _wrap_row_stride = kNWarps * 16;
        __nv_bfloat16* bias_ptr = reinterpret_cast<__nv_bfloat16*>(params.kv_bias);
        #pragma unroll
        for (int mi = 0; mi < size<0, 1>(acc_s_4d); ++mi) {
            #pragma unroll
            for (int i = 0; i < size<0, 0>(acc_s_4d); ++i) {
                #pragma unroll
                for (int nj = 0; nj < size<1, 1>(acc_s_4d); ++nj) {
                    const int _col_idx_base = _col_idx_offset + nj * 8;
                    #pragma unroll
                    for (int j = 0; j < size<1, 0>(acc_s_4d); ++j) {
                        const int _col_idx = _col_idx_base + j;
                        const int index = 
                            bidb * params.kv_bias_batch_stride + 
                            bidh * params.kv_bias_head_stride + 
                            _col_idx * params.kv_bias_row_stride;
                        acc_s_4d(make_coord(i, mi), make_coord(j, nj)) += __bfloat162float(bias_ptr[index]);

                        // if (__bfloat162float(bias_ptr[index]) != 0.0) {   // 只让 threadIdx.x == 0 打印，避免 1000 行输出
                        // printf(
                        //     "[bias_added] b=%d h=%d global_col_idx=%d  "
                        //     "coord(i=%d, mi=%d, j=%d, nj=%d)  bias=%f\n",
                        //     bidb, bidh, _col_idx,
                        //     i, mi, j, nj,
                        //     __bfloat162float(bias_ptr[index])
                        // );
                        // }
