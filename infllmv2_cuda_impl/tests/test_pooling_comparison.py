...
def _artifact_dir(env_var):
    ...
    if not value:
        pytest.skip(f"Set {env_var} to run this optional artifact-based test.")
...
def test_pooling_functions():
    ...
    data_dir = _artifact_dir("INFLLMV2_POOLING_COMPARISON_DIR")
    
    print(f"Loading data from {data_dir}")
    attn_score_full = torch.load(data_dir / "attn_score.pt").to(torch.bfloat16)
    cu_seqlens_q_full = torch.load(data_dir / "cu_seqlens_q.pt")
    cu_seqlens_k_full = torch.load(data_dir / "cu_seqlens_k.pt")
    max_seqlen_q = torch.load(data_dir / "max_seqlen_q.pt")
    max_seqlen_k = torch.load(data_dir / "max_seqlen_k.pt")
