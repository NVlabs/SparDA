...
def test_flash_attn_varlen(seqlen_q=256, seqlen_k=16, n_heads=32, n_kv_heads=2, head_dim=128, dtype=torch.bfloat16, bench=False, causal=False, batch_size=1):
    ...
    if total_seqlen_k == 0:
        pytest.skip("compressed key sequence is empty for this debug-size configuration")
    # breakpoint()
    # 朴素实现
    if not bench:
    ...
    if False:
        ...
        naive_diff = (naive_score - flash_score).abs().max()
        print("score max diff :", naive_diff)
        # breakpoint()
        torch_diff = (torch_score - flash_score).abs().max()
        print("torch score max diff :", torch_diff)
        
        assert naive_diff <= 2e-2, f"seqlen_qs={seqlen_qs}, seqlen_ks={seqlen_ks}"
