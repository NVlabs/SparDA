...
def _artifact_dir(env_var):
    ...
    if not value:
        raise RuntimeError(f"Set {env_var} to run this optional debug script.")
    path = Path(value).expanduser()
    if not path.is_dir():
        raise RuntimeError(f"{env_var} does not point to an existing directory: {path}")
    return path
...
def main():
    tp1_dir = _artifact_dir("INFLLMV2_TP1_DIR")
    tp2_rank0_dir = _artifact_dir("INFLLMV2_TP2_RANK0_DIR")
    tp2_rank1_dir = _artifact_dir("INFLLMV2_TP2_RANK1_DIR")
    
    # Load the blockmask files
    print("Loading blockmask files...")
    
    try:
        mask_tp1 = torch.load(tp1_dir / "fwd_blockmask_bool.pt")
    ...
    try:
        mask_tp2_rank0 = torch.load(tp2_rank0_dir / "fwd_blockmask_bool.pt")
    ...
    try:
        mask_tp2_rank1 = torch.load(tp2_rank1_dir / "fwd_blockmask_bool.pt")
