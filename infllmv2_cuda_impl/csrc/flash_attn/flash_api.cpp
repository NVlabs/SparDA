...
void set_params_fprop(Flash_fwd_params &params,
    ...
    params.stage1_causal_stride = 16;
    params.stage1_query_offset = 0;
    params.stage1_kl_teacher_mode = false;
...
mha_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x round_multiple(head_size, 8)
    ...
    bool is_sm10x = cc_major >= 10;
    TORCH_CHECK(is_sm10x || is_sm90 || is_sm8x, "FlashAttention only supports Ampere GPUs or newer.");
    ...
    if (q_dtype == torch::kBFloat16) {
        TORCH_CHECK(is_sm10x || is_sm90 || is_sm8x, "bfloat16 is only supported on Ampere GPUs or newer");
    }
...
mha_varlen_fwd(at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    if (max_seqlen_q == 1 && !alibi_slopes_.has_value()) {
        is_causal = false;  // causal=true is the same as causal=false in this case
    }
    if (is_causal) { window_size_right = 0; }
...
std::vector<at::Tensor>
mha_varlen_fwd_stage1_impl(at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
               const at::Tensor &k,  // total_k x num_heads_k x head_size, total_k := \sum_{i=0}^{b} s_i or num_blocks x page_block_size x num_heads_k x head_size if there's a block_table.
               const at::Tensor &v,  // total_k x num_heads_k x head_size, total_k := \sum_{i=0}^{b} s_i or num_blocks x page_block_size x num_heads_k x head_size if there's a block_table.
               c10::optional<at::Tensor> &out_, // total_q x num_heads x head_size, total_k := \sum_{i=0}^{b} s_i
               const at::Tensor &cu_seqlens_q,  // b+1
               ...
               const bool return_softmax,
               c10::optional<at::Generator> gen_,
               const int stage1_causal_stride,
               const int stage1_query_offset,
               const bool stage1_kl_teacher_mode) {
    ...
    if (max_seqlen_q == 1 && !alibi_slopes_.has_value() && !stage1_kl_teacher_mode) {
        is_causal = false;  // Only valid for the legacy right-aligned stage1 mask.
    }
    if (is_causal) { window_size_right = 0; }
    ...
    params.m_block_dim = 16;
    params.n_block_dim = 1;
    params.stage1_causal_stride = stage1_causal_stride;
    params.stage1_query_offset = stage1_query_offset;
    params.stage1_kl_teacher_mode = stage1_kl_teacher_mode;
...
std::vector<at::Tensor>
mha_varlen_fwd_stage1(at::Tensor &q,
               const at::Tensor &k,
               const at::Tensor &v,
               ...
               const bool return_softmax,
               c10::optional<at::Generator> gen_) {
    return mha_varlen_fwd_stage1_impl(
        q, k, v, out_, cu_seqlens_q, cu_seqlens_k, cu_seqlens_v, seqused_k, leftpad_k_,
        block_table_, alibi_slopes_, max_seqlen_q, max_seqlen_k, p_dropout, softmax_scale,
        zero_tensors, is_causal, window_size_left, window_size_right, softcap, return_softmax,
        gen_, /*stage1_causal_stride=*/16, /*stage1_query_offset=*/0, /*stage1_kl_teacher_mode=*/false
    );
}

std::vector<at::Tensor>
mha_varlen_fwd_stage1_kl_teacher(at::Tensor &q,
               const at::Tensor &k,
               const at::Tensor &v,
               c10::optional<at::Tensor> &out_,
               const at::Tensor &cu_seqlens_q,
               ...
               const bool return_softmax,
               c10::optional<at::Generator> gen_,
               const int causal_stride,
               const int query_offset) {
    TORCH_CHECK(causal_stride > 0, "causal_stride must be positive");
    TORCH_CHECK(query_offset >= 0, "query_offset must be non-negative");
    return mha_varlen_fwd_stage1_impl(
        q, k, v, out_, cu_seqlens_q, cu_seqlens_k, cu_seqlens_v, seqused_k, leftpad_k_,
        block_table_, alibi_slopes_, max_seqlen_q, max_seqlen_k, p_dropout, softmax_scale,
        zero_tensors, is_causal, window_size_left, window_size_right, softcap, return_softmax,
        gen_, causal_stride, query_offset, /*stage1_kl_teacher_mode=*/true
    );
}
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x multiple_of(head_size_og, 8)
    ...
    if (head_size > 192 && is_dropout) {
        TORCH_CHECK(is_sm10x || is_sm80 || is_sm90, "FlashAttention backward for head dim > 192 with dropout requires A100/A800, H100/H800, or newer");
    }
    TORCH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
