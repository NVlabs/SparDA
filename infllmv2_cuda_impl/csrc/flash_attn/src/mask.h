...
struct Mask {
    ...
    // Causal_mask: whether this particular iteration needs causal masking
    // causal_k_offset: right-alignment offset so the last query sees all keys
    //   (= actual_seqlen_k - (orig_seqlen_q - stride + 1) / stride).
    //   Zero when k_len matches the default stride-16 compressed q_len.
    template <bool Causal_mask=false, bool Is_even_MN=true, typename Engine, typename Layout>
    __forceinline__ __device__ void apply_mask_stage1(Tensor<Engine, Layout> &tensor_,
                                               ...
                                               const int warp_row_stride,
                                               const int stride = 16,
                                               const int causal_k_offset = 0,
                                               const int query_row_offset = 0,
                                               const bool kl_teacher_mode = false) {
        static_assert(!(Causal_mask && Is_local), "Cannot be both causal and local");
        static_assert(Layout::rank == 3, "Only support 3D Tensor");
        static_assert(decltype(size<0>(tensor_))::value == 4, "First dimension must be 4");
        static constexpr bool Need_masking = Has_alibi || Causal_mask || Is_local || !Is_even_MN;
            ...
            if constexpr (Col_idx_only) {
                ...
                for (int mi = 0; mi < size<0, 1>(tensor); ++mi) {
                    ...
                    for (int i = 0; i < size<0, 0>(tensor); ++i) {
                        ...
                        const int orig_row_idx = row_idx / this->m_block_dim;
                        const int logical_row_idx = orig_row_idx + query_row_offset;
                        const int teacher_col_idx = logical_row_idx / stride;
                        const int teacher_col_idx_exclusive = teacher_col_idx + 1;

                        const int col_idx_limit_left = kl_teacher_mode
                            ? std::max(0, teacher_col_idx - window_size_left)
                            : std::max(0, (orig_row_idx - stride + 1) / stride + causal_k_offset - window_size_left);
                        const int col_idx_limit_right = kl_teacher_mode
                            ? std::min(max_seqlen_k, teacher_col_idx_exclusive + window_size_right)
                            : std::min(max_seqlen_k, (orig_row_idx - stride + 1) / stride + causal_k_offset + window_size_right);
                        #pragma unroll
                        for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                            const int col_idx_base = col_idx_offset + nj * 8;
                            #pragma unroll
