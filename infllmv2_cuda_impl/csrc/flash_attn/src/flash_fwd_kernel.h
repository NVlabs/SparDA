...
inline __device__ void compute_attn_1rowblock_splitkv_stage1(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    const int n_block_min = !Is_local
        ...
        : std::max(n_split_idx * n_blocks_per_split, (m_block * kBlockM + binfo.actual_seqlen_k - binfo.actual_seqlen_q - params.window_size_left) / kBlockN);
    int n_block_max = std::min(cute::ceil_div(binfo.actual_seqlen_k, kBlockN), (n_split_idx + 1) * n_blocks_per_split);
    // The inference stage1 path keeps its historical right-aligned causal rule.
    // KL-teacher mode instead uses exact start-position semantics with a caller-
    // supplied stride and absolute query offset.
    const int stage1_stride = params.stage1_kl_teacher_mode ? params.stage1_causal_stride : 16;
    const int stage1_query_offset = params.stage1_kl_teacher_mode ? params.stage1_query_offset : 0;
    const int _ori_seqlen_q = binfo.actual_seqlen_q / params.m_block_dim;
    const int _k_actual_seqlen_q = (_ori_seqlen_q - 16 + 1) / 16;
    const int _offset_k = binfo.actual_seqlen_k - _k_actual_seqlen_q;
    if (Is_causal || Is_local) {
        const int max_q = (m_block + 1) * kBlockM / params.m_block_dim - 1;
        const int max_k = params.stage1_kl_teacher_mode
            ? (stage1_query_offset + max_q) / stage1_stride + 1
            : (max_q - 16 + 1) / 16 + _offset_k;
        n_block_max = std::min(n_block_max,
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (!skip) {
            ...
            mask.template apply_mask_stage1<Is_causal, Is_even_MN>(
                ...
                /*causal_k_offset=*/params.stage1_kl_teacher_mode ? 0 : _offset_k,
                /*query_row_offset=*/stage1_query_offset,
                /*kl_teacher_mode=*/params.stage1_kl_teacher_mode
            );
