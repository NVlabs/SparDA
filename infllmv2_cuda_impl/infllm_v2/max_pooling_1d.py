...
def max_pooling_1d_varlen(
    ...
    if buf is None:
        ...
        assert buf.shape == (num_heads, total_q, out_len), (
            f"buf shape mismatch: expected {(num_heads, total_q, out_len)}, got {tuple(buf.shape)}"
        )
        assert buf.dtype == input.dtype, f"buf dtype mismatch: expected {input.dtype}, got {buf.dtype}"
        assert buf.device == input.device, f"buf device mismatch: expected {input.device}, got {buf.device}"
